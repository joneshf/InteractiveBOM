...
drawtext :: forall a. EffectFn4 Context2D Text String a Unit
drawtext = mkEffectFn4 \ctx text color _ -> case text.pos !! 0, text.pos !! 1 of
  Just translateX, Just translateY -> do
    save ctx
    translate ctx { translateX, translateY }
    ...
    forWithIndex_ txt \i txti -> do
      let lineWidth = maybe 0.0 _.offsetx (last characters)
          offsety =
            toNumber (i * 2 - length txt + 1) * interline + text.height / 2.0
          characters = mkCharacters text txti
          justify = case text.horiz_justify of
            ...
            _ -> 0.0
      for_ characters \{ l, offsetx, width } -> do
...
mkCharacters :: forall r. { width :: Number | r } -> String -> Array Character
mkCharacters text str =
